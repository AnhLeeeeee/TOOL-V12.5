...
public sealed class ChromeController : IAsyncDisposable
{
    ...
    public async Task LaunchAsync(int port, string profileDir, Action? beforeLaunch = null)
    {
        ...
        _log.Info($"Đã mở Chrome V11 ở chế độ HIỂN THỊ; cổng CDP={port}; profile={profileDir}");
    ...
    CdpClient Cdp => _cdp ?? throw new InvalidOperationException("Chưa kết nối Chrome V11.");
...
""", ct: ct);
    ...
    public async Task<byte[]> CaptureViewportAsync(CancellationToken ct = default)
    {
        LogCdpStart("CaptureViewport", "Page.captureScreenshot");
        var r = await Cdp.CallAsync("Page.captureScreenshot", new { format = "png", fromSurface = true, captureBeyondViewport = false }, ct);
        var bytes = Convert.FromBase64String(r.GetProperty("data").GetString() ?? "");
        LogCdpDone("CaptureViewport", $"{bytes.Length} bytes");
        return bytes;
    }

    public async Task<byte[]> CaptureXPathAsync(string xpath, CancellationToken ct = default)
    {
        LogCdpStart("CaptureXPath", TrimForLog(xpath));
        var box = await GetBoxNoScrollAsync(xpath, ct) ?? throw new InvalidOperationException("Không tìm thấy XPath để chụp: " + xpath);
        var (vw, vh) = await GetViewportSizeAsync(ct);
        var x = Math.Max(0, box.X);
        var y = Math.Max(0, box.Y);
        var width = Math.Min(box.Width - Math.Max(0, -box.X), Math.Max(0, vw - x));
        var height = Math.Min(box.Height - Math.Max(0, -box.Y), Math.Max(0, vh - y));
        if (width < 2 || height < 2)
            throw new InvalidOperationException("Element XPath đang nằm ngoài viewport hoặc có kích thước quá nhỏ để quét. Tool không tự cuộn trang để tránh Chrome bị giật.");
        var r = await Cdp.CallAsync("Page.captureScreenshot", new
        {
            format = "png",
            fromSurface = true,
        ...
        var bytes = Convert.FromBase64String(r.GetProperty("data").GetString() ?? "");
        LogCdpDone("CaptureXPath", $"{bytes.Length} bytes | {TrimForLog(xpath)}");
        return bytes;
    }
