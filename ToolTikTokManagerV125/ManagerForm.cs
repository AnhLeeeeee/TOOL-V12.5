...
public sealed class ManagerForm : Form
{
    ...
    public ManagerForm()
    {
        LegacyDataMigration.TryImportV12Catalog(_baseDir);
        _profileService = new TikTokProfileService(_baseDir);
        _log = new Logger(_baseDir, "manager", "manager-v125.log");
        Text = "Tool TikTok Manager V12.5 — V11.5 Multi Worker";
        Width = 1440;
        Height = 900;
    ...
    void EnsureTab(ProfileContext ctx)
    {
        ...
        var detach = Button("Tách V11.5", (_, _) => ToggleDetach(ctx));
    ...
    async Task<bool> OpenProfileAsync(ProfileContext ctx, string? openingStatus = null)
    {
        ...
        try
        {
            ...
            LegacyDataMigration.TryImportV12ProfileData(_baseDir, ctx.Profile.Name, _profileService.ResolveDataRoot(ctx.Profile));
    ...
    async Task EnsureWorkerAsync(ProfileContext ctx)
    {
        ...
        if (!process.Start()) throw new InvalidOperationException("Không khởi động được V11.5 worker.");
        ctx.Worker = process;
        ...
        throw new TimeoutException("Worker V11.5 chưa mở pipe sau 10 giây.");
    }
    ...
    async Task EmbedWorkerAsync(ProfileContext ctx)
    {
        ...
        if (!WorkerWindowEmbedder.IsValid(ctx.WorkerWindow)) throw new InvalidOperationException("Không lấy được cửa sổ V11.5 worker.");
        ctx.Host.CreateControl();
        if (!WorkerWindowEmbedder.Attach(ctx.WorkerWindow, ctx.Host)) throw new InvalidOperationException("Không gắn được giao diện V11.5 vào tab.");
        ctx.Detached = false;
        if (ctx.DetachButton is not null) ctx.DetachButton.Text = "Tách V11.5";
    }
    ...
    void ToggleDetach(ProfileContext ctx)
    {
        try
        {
            ...
            if (!ctx.Detached)
            {
                ...
                {
                    ctx.Detached = true;
                    if (ctx.DetachButton is not null) ctx.DetachButton.Text = "Gắn V11.5";
                }
            }
    ...
    void EnsureSafeDeletionTargets(TikTokProfileEntry profile, string chromePath, string dataRoot)
    {
        ...
        if (!IsChildPathOf(dataRoot, dataRootBase))
            throw new InvalidOperationException($"[DELETE_PATH_BLOCKED] Profile “{profile.Name}” có DataRoot ngoài vùng dữ liệu V12.5: {dataRoot}. Không xóa cấu hình hay thư mục nào.");
    ...
    {
        var selected = SelectedContext();
        Text = selected is null ? "Tool TikTok Manager V12.5 — V11.5 Multi Worker" : $"Tool TikTok Manager V12.5 — {selected.Profile.Name}";
        RefreshSelectedProfilePresentation();
    }
    ...
    void ShowError(Exception ex)
    {
        ...
        MessageBox.Show(ex.Message, "V12.5", MessageBoxButtons.OK, MessageBoxIcon.Warning);
    }
    ...
    string ResolveWorkerExe()
    {
        var candidates = new[]
        {
            Path.Combine(_baseDir, "ToolTikTokWorkerV125.exe"),
            Path.Combine(_baseDir, "ToolTikTokWorkerV125")
        };
        return candidates.FirstOrDefault(File.Exists) ?? throw new FileNotFoundException("Không tìm thấy ToolTikTokWorkerV125.exe trong dist_v125.");
    }

    static string PipeName(string profileName) => "ToolTikTokV125_" + profileName;
